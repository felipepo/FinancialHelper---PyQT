...
class Create(QtWidgets.QWidget):
    def __init__(self, parent):
        ...
        ## Initialization ==
        ## Creation ==
        self.cardArea = cards.CardArea(self, 1)
...
class SetValueGroup(QtWidgets.QGroupBox):
    ...
    def SetFinalValue(self):
        ...
        Acc_ID = self.controlFrame.accPage.mainWin.DataBase.acc_tbl.readByUnique(1, accName)
        Catg_ID = self.controlFrame.accPage.mainWin.DataBase.category_tbl.readByName("Outros")
        diffValue = int(self.finalValue.text().replace('.','')) - self.controlFrame.accPage.mainWin.DataBase.Totals["debit"][accName]
        if diffValue != 0:
            transData = {
                "Catg_ID": Catg_ID[0],
                "Category": "Outros",
...
class FilterGroup(QtWidgets.QGroupBox):
    ...
    def getComboValues(self):
        catgTable = self.controlFrame.accPage.mainWin.DataBase.category_total_tbl.readAll()
        years = ["Todos"]
