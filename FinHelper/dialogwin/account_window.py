...
class New(QtWidgets.QDialog):
    def __init__(self, parent):
        ...
        self.closingDayEdit = QtWidgets.QLineEdit(self.groupBox, objectName="newEdit", placeholderText="00", alignment=QtCore.Qt.AlignCenter, maxLength=2)
        self.dueDatLbl = QtWidgets.QLabel(self.groupBox, objectName="dueDatLbl", text="Dia de Vencimento")
        self.dueDatEdit = QtWidgets.QLineEdit(self.groupBox, objectName="newEdit", placeholderText="00", alignment=QtCore.Qt.AlignCenter, maxLength=2)
        ...
        self.initialValueEdit.textChanged.connect(lambda state, targetEdit=self.initialValueEdit: self.numberEntered(targetEdit))
        self.limitEdit.textChanged.connect(lambda state, targetEdit=self.limitEdit: self.numberEntered(targetEdit))

        # dayRegexp = QtCore.QRegExp(r'(0[1-9]|[12]\d|3[01])')
        # validator = QtGui.QRegExpValidator(dayRegexp)
        # self.closingDayEdit.setValidator(validator)
        # self.closingDayEdit.textChanged.connect(self.check_state)
